...
function LoginForm() {
  ...
  return <form onSubmit={(event) => void submit(event)} className="w-full max-w-sm rounded-2xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.03)] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.34)]"><div className="flex h-11 w-11 items-center justify-center rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(94,106,210,0.18)]"><LockKeyhole size={20}/></div><h1 className="mt-5 text-2xl font-[510] tracking-[-0.704px]">Tomotono admin login</h1><p className="mt-2 text-sm leading-6 text-[var(--text-tertiary)]">Delivery route data is protected by an MVP admin gate. Set ADMIN_PASSWORD before deployment.</p><input value={password} onChange={(event) => setPassword(event.target.value)} type="password" placeholder="Admin password" className="mt-5 h-10 w-full rounded-md border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.02)] px-3 text-sm outline-none focus:border-[var(--accent-violet)]" />{error ? <p className="mt-3 text-xs text-red-300">{error}</p> : null}<Button className="mt-5 w-full" variant="primary">Login</Button></form>;
}
